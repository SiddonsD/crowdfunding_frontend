async function getProjects() {
    ...
    // Next we call the fetch function and pass in the url and the method. 
    // The method is set to `GET` because we are fetching data. Fetch returns a"promise".
    // If the promise "resolves" (i.e., if the back end responds) we will get the data we need in the `response` variable. 
    // If the back end fails to respond then we'll get an error.
